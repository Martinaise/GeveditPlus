...
<?php
    ...
    if ($_GET && $_GET['action'] == 'supprimer') {
      $controller->supprimerProduitParId($_GET['id_produit']);
  ...
  <table class="table">
    ...
    <tbody>
      <?php foreach($produits as $result) {
        ...
        echo ('<td><a class="btn btn-dark" href="modificationproduit.php?id_produit=' . $result['id_produit'] . '">Modifier</a> | <a class="btn btn-dark" href="?action=supprimer&&id_produit=' . $result['id_produit'] . '">Supprimer</a></td>');
        echo '</tr>';
      }
      ?>
    </tbody>
